...
* **emonTH_DHT22_multiple_DS18B20** - derived from the dual sensor emonTH firmware by Marshall Scholz. Capable of automatically discovering and monitoring any connected DS18B20 sensors. The only downfall of this version is that sensor addresses could possibly change if an extra sensor is added once the node has been set up. (This can be easily rectified by changing the input logging feed in emonCMS)
