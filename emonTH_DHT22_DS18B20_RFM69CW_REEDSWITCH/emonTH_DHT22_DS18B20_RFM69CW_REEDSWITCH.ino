/*
  emonTH Low Power DHT22 Humidity & Temperature & DS18B20 Temperature & Reed Switch Pulse  Node Example 
  ...
  It counts pulses coming form a reed switch
  
  Technical hardware documentation wiki: https://wiki.openenergymonitor.org/index.php/EmonTH_V1.5
  Add a pull down resistor between port 3 (GND) and 4 (Pulse)
  Connect your reed switch betwen 4 (Pulse) and 2 (3.3V)  
  ...
  Authors: Eric AMANN. 
  Derived from Glyn Hudson's sketch (emonTH_DHT22_DS18B20_RFM69CW sketch - V1.6.1)
...
;
  Change log:
  V1.0         add support for counting pulses from a reed switch to the emonTH_DHT22_DS18B20_RFM69CW sketch (V1.6.1)
  
emonhub.conf nodes config:

[[19]]
    nodename = emonTH_1
    firmware = emonTH_DHT22_DS18B20_RFM69CW_REEDSWITCH
    hardware = emonTH_(Node_ID_Switch_DIP1:OFF_DIP2:OFF)
    [[[rx]]]
       names = temperature, external temperature, humidity, pulse, battery
       datacode = h
       scales = 0.1,0.1,0.1,1,0.1
       units = C,C,%,P,V
*/
...
const int polling_period = 500;                                              // Polling period in ms (check the state of the reed switch)
...
//################################################################################################################################
  ...
  if (debug==1)
  {
    ...
    Serial.println("emonTH_DHT22_DS18B20_RFM69CW_REEDSWITCH V1.0"); 
